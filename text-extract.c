...
static int tocents = 0;
static entry_t *toc = NULL;
...
int extracttext(char *strlistname, char *outname, uint8_t *rombuf2, size_t romsize2)
{
	rombuf = rombuf2;
	romsize = romsize2;
	
  /* ---------- build output file TOC ---------- */
  FILE *f = fopen(strlistname, "rb");
  if (!f)
  {
    printf("Couldn't open \"%s\": %s\n", strlistname,strerror(errno));
    return EXIT_FAILURE;
  }
  lexinit(f, strlistname);
  int lextype = yylex();
  while (lextype)
  {
    if (lextype != TOK_ID)
